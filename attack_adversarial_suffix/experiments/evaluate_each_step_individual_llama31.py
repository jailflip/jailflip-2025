...
def main(_):
    ...
    seed = params.logfile[params.logfile.index('_seed')+len('_seed') : params.logfile.index('_1E')]
    ...
    # --------------debugging: eval all the stored control suffixs which is saved in the json file --------------#
    ...
    for model in _MODELS:
        ...
        total_jb, total_em, test_total_jb, test_total_em, total_outputs, test_total_outputs = attack.run(
            range(len(controls)),       # this item isn't used in the run function
            controls,
            batch_size,
            max_new_len=params.eval_max_new_len          # for the model.generate
        )
        # ------------------------------------------------------------------------------------------------ #
        # the above six object returned from the attck function are all lists                              #
        # in shape #controls, #train/test data                                                             #
        # ------------------------------------------------------------------------------------------------ #
        ...
        for i in range(len(controls)):
            ctrl_index = f"The control string at {525-25*len(controls)+25*i} step "
            temp_train_acc = train_acc[i]
            temp_total_jb = total_jb[i]
            temp_test_total_jb = test_total_jb[i]
            results[ctrl_index+'results overview'] = f"train jf asr:{np.array(temp_train_acc).sum()}    train jb:{np.array(temp_total_jb).sum()}    test jb:{np.array(temp_test_total_jb).sum()}"
            train_acc_overview[525-25*len(controls)+25*i] = int(np.array(temp_train_acc).sum())
            train_jb_overview[525-25*len(controls)+25*i] = int(np.array(temp_total_jb).sum())
            test_jb_overview[525-25*len(controls)+25*i] = int(np.array(temp_test_total_jb).sum())
